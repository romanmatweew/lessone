...
class MainWindow(tk.Frame):
    def __init__(self, parent):
        ...
        self.principalScale = tk.Scale(self, from_=0, to=10000, resolution=100, orient=tk.HORIZONTAL, variable=self.principal, command=self.update_ui)
        ...
        self.rateScale = tk.Scale(self, from_=0, to=20, resolution=1, orient=tk.HORIZONTAL, variable=self.rate, command=self.update_ui)
        self.rateScale.grid(row=1, column=1)

        yearsLabel = tk.Label(self, text="Years:")
        yearsLabel.grid(row=2, column=0, sticky=tk.W, padx=10)

        self.yearsScale = tk.Scale(self, from_=0, to=50, resolution=1, orient=tk.HORIZONTAL, variable=self.years, command=self.update_ui)
